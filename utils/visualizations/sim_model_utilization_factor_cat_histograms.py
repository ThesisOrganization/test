...
def plot(files_list, name_files, path_out):
    ...
    for list_policy_files in files_list:
        ...
        max_value = np.amax(final_result)
        PERCENTAGE = 0.1
        list_data_to_plot.append((np.transpose(final_result), names, "Sim vs Model Utilization factor", "Element Type", "% difference in U", 0.0, max_value + max_value*PERCENTAGE, ["Telemetry", "Transition", "Command", "Batch"], path_out + directory_name + name_files[index_file][0] + "_" + name_files[index_file][1]))
