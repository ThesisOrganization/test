...
def group_node_computation(groups, model_groups, type_element):
    ...
    for group in groups:
        ...
        tel_model = np.array(jmespath.search("[*].parameters.telemetry.response_time", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].parameters.transition.response_time", model_groups[number]))
        com_model = np.array(jmespath.search("[*].parameters.command.response_time", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].parameters.batch.response_time", model_groups[number]))
...
def group_lan_computation(groups, model_groups, type_element):
    ...
    for group in groups:
        ...
        tel_model = np.array(jmespath.search("[*].lan_in.telemetry.response_time", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].lan_in.transition.response_time", model_groups[number]))
        com_model = np.array(jmespath.search("[*].lan_in.command.response_time", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].lan_in.batch.response_time", model_groups[number]))
        ...
        tel_model = np.array(jmespath.search("[*].lan_out.telemetry.response_time", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].lan_out.transition.response_time", model_groups[number]))
        com_model = np.array(jmespath.search("[*].lan_out.command.response_time", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].lan_out.batch.response_time", model_groups[number]))
...
def plot(files_list, name_files, path_out):
    ...
    directory_name = "sim_model_response_timeB_histograms/"
    ...
    for list_policy_files in files_list:
        ...
        max_value = 1.0
        PERCENTAGE = 0.0
        draw_grouped_histograms(np.transpose(final_result), names, "Sim vs Model Response time B", "Element Type", "% difference in RB", 0.0, max_value + max_value*PERCENTAGE, ["Telemetry", "Transition", "Command", "Batch"], PATH=path_out + directory_name + name_files[index_file][0] + "_" + name_files[index_file][1])
