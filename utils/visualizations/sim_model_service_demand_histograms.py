...
def group_node_computation(groups, model_groups, type_element):
    ...
    number = 0
    for group in groups:
        tel = np.array(jmespath.search("[*].parameters.telemetry.service_demand", group))
        tra = np.array(jmespath.search("[*].parameters.transition.service_demand", group))
        com = np.array(jmespath.search("[*].parameters.command.service_demand", group))
        bat = np.array(jmespath.search("[*].parameters.batch.service_demand", group))
        ...
        tel_model = np.array(jmespath.search("[*].parameters.telemetry.service_demand", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].parameters.transition.service_demand", model_groups[number]))
        com_model = np.array(jmespath.search("[*].parameters.command.service_demand", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].parameters.batch.service_demand", model_groups[number]))
...
def group_lan_computation(groups, model_groups, type_element):
    ...
    number = 0
    for group in groups:
        tel = np.array(jmespath.search("[*].lan_in.telemetry.service_demand", group))
        tra = np.array(jmespath.search("[*].lan_in.transition.service_demand", group))
        com = np.array(jmespath.search("[*].lan_in.command.service_demand", group))
        bat = np.array(jmespath.search("[*].lan_in.batch.service_demand", group))
        ...
        tel_model = np.array(jmespath.search("[*].lan_in.telemetry.service_demand", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].lan_in.transition.service_demand", model_groups[number]))
        com_model = np.array(jmespath.search("[*].lan_in.command.service_demand", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].lan_in.batch.service_demand", model_groups[number]))
        ...
        tel = np.array(jmespath.search("[*].lan_out.telemetry.service_demand", group))
        tra = np.array(jmespath.search("[*].lan_out.transition.service_demand", group))
        com = np.array(jmespath.search("[*].lan_out.command.service_demand", group))
        bat = np.array(jmespath.search("[*].lan_out.batch.service_demand", group))
        ...
        tel_model = np.array(jmespath.search("[*].lan_out.telemetry.service_demand", model_groups[number]))
        tra_model = np.array(jmespath.search("[*].lan_out.transition.service_demand", model_groups[number]))
        com_model = np.array(jmespath.search("[*].lan_out.command.service_demand", model_groups[number]))
        bat_model = np.array(jmespath.search("[*].lan_out.batch.service_demand", model_groups[number]))
...
def plot(files_list, name_files, path_out):
    ...
    directory_name = "sim_model_service_demand_histograms/"
    ...
    for list_policy_files in files_list:
        ...
        max_value = np.amax(final_result)
        PERCENTAGE = 0.1
        list_data_to_plot.append((np.transpose(final_result), names, "Sim vs Model Service demand", "Element Type", "% difference of D", 0.0, max_value + max_value*PERCENTAGE, ["Telemetry", "Transition", "Command", "Batch"], path_out + directory_name + name_files[index_file][0] + "_" + name_files[index_file][1]))
