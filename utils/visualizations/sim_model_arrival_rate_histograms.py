...
def group_node_computation(groups, model_groups, type_element):
    ...
    number = 0
    for group in groups:
        tel = np.array(jmespath.search("[*].parameters.telemetry.lambda_in", group))
        tra = np.array(jmespath.search("[*].parameters.transition.lambda_in", group))
        com = np.array(jmespath.search("[*].parameters.command.lambda_in", group))
        bat = np.array(jmespath.search("[*].parameters.batch.lambda_in", group))
...
def group_lan_computation(groups, model_groups, type_element):
    ...
    number = 0
    for group in groups:
        tel = np.array(jmespath.search("[*].lan_in.telemetry.lambda_in", group))
        tra = np.array(jmespath.search("[*].lan_in.transition.lambda_in", group))
        com = np.array(jmespath.search("[*].lan_in.command.lambda_in", group))
        bat = np.array(jmespath.search("[*].lan_in.batch.lambda_in", group))
        ...
        com = np.array(jmespath.search("[*].lan_out.command.lambda_in", group))
        bat = np.array(jmespath.search("[*].lan_out.batch.lambda_in", group))
...
def plot(files_list, name_files, path_out):
    ...
    for list_policy_files in files_list:
        ...
        max_value = 1.0
        PERCENTAGE = 0.0
        draw_grouped_histograms(np.transpose(final_result), names, "Sim vs Model arrival rate", "Element Type", "% difference in Lambda", 0.0, max_value + max_value*PERCENTAGE, ["Telemetry", "Transition", "Command", "Batch"], PATH=path_out + directory_name + name_files[index_file][0] + "_" + name_files[index_file][1])
