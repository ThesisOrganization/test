...
def plot(files_list, name_files, path_out):
    ...
    directory_name = "sim_model_RA_over_U_complete/"
    ...
    for list_policy_files in files_list:
        ...
        for path_file in list_policy_files:
            ...
            for i in range(0, len_data):
                ...
                if sim_element["type"] != "lan":
                    ...
                    value_model = np.array(jmespath.search("parameters.*.response_time_a", model_element))
                    ...
                    value_model = np.array(jmespath.search("*.*.response_time_a", model_element))
                    diff = np.absolute(value_sim - value_model)
        ...
        list_data_to_plot.append((list(final_result), list(final_result_x), ["Telemetry", "Transition", "Command", "Batch"], "% difference of RA against U", "U", "% difference of R", 0.0, max_value + PERCENTAGE*max_value, path_out + directory_name + name_files[index_file][0] + "_" + name_files[index_file][1]))
